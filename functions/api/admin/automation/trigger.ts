// POST /api/admin/automation/trigger — set trigger_requested flag
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const key = `automation.${job}.trigger_requested`;
  ...
  await logAudit(db, user, 'automation.trigger', `Triggered ${job}`, request);

  return jsonResponse({ ok: true, message: `Trigger set for ${job}. Will run at next scheduled time.` });
};
