// POST /api/admin/automation/pause — set paused flag
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  await logAudit(db, user, 'automation.pause', `Paused ${job}`, request);

  return jsonResponse({ ok: true, message: `${job} paused` });
};
